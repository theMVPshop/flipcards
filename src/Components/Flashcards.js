...
const Flashcards = () => {
  ...
  const api = "https://flipcardzdb.herokuapp.com"

  useEffect(() => {
    async function fetchData() {
      try {
        let res, data, cardSets
        res = await axios.get(`${api}/card/all-flashcards`)
  ...
  const changeCard = (direction) => {
    ...
    let newIdx
    direction === "previous" ? (newIdx = currentCardIdx - 1) : (newIdx = currentCardIdx + 1)
